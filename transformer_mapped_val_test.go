...
func TestMappedValueCanSetMetadata(t *testing.T) {
	data, err := filtrify.ConvertToTypedData(test.UATMappedValueTestDataFormatted, true, true)
	if err != nil {
		assert.NoError(t, err, "basic data conversion failed")
	}
	...
	conf := &operator.MappedValueConfiguration{
		...
		MappedColumnName: "Broker ID",
		NewColumnName:    "Broker Mapped",
		NewColumnMeta:    map[string]interface{}{"type": "test"},
	}
	b1, err := json.Marshal(conf)
	if err != nil {
		panic(err.Error())
	...
	// let's get new column header
	header := joinedData.Headers[conf.NewColumnName]
	assert.NotNil(t, header, "new column header was not found")
	assert.NotNil(t, header.Metadata, "new column metadata was not found")
	assert.Equal(t, header.ColumnName, conf.NewColumnName, "invalid column name")
	assert.Equal(t, header.DataType, types.StringType, "invalid data type")
	assert.Equal(t, "test", header.Metadata["type"], "invalid metadata")
}
