...
func (m *MIN) Validate(n *expr.FuncNode) (expr.EvaluatorFunc, error) {
	if len(n.Args) != 2 {
		return nil, fmt.Errorf("expected 2 arg for MIN(arg) but got %s", n)
	}
	return minEval, nil
}

func minEval(ctx expr.EvalContext, args []value.Value) (value.Value, bool) {
...
func (m *MIN) Type() value.ValueType { return value.NumberType }
